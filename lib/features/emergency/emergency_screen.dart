              ...
              child: Column(
                ...
                children: [
                   const Icon(Icons.warning_amber_rounded, size: 64, color: AppColors.error)
                      .animate().shake(hz: 4, duration: 800.ms),
                   ...
                   Container(
                     ...
                     child: const Column(
                       ...
                       children: [
                         Text('Detected Entry: JKIA', style: TextStyle(fontWeight: FontWeight.bold)),
                         SizedBox(height: 8),
                         Text('Current Exit: Westlands', style: TextStyle(fontWeight: FontWeight.bold)),
                         Divider(color: AppColors.error),
                         Text('Max Default Fee Applied', style: TextStyle(color: Colors.redAccent, fontSize: 12)),
                       ],
                     ),
                   ),
                   const SizedBox(height: 24),
                   ...
                   if (status == PaymentStatus.pending)
                     Column(
                       children: [
                         ...
                         const SizedBox(height: 16),
                         const Text('Waiting for M-Pesa PIN...', style: TextStyle(color: AppColors.error))
                     ],
                   ).animate().fade()
                   else
                     ElevatedButton.icon(
                       onPressed: () {
                         ref.read(paymentProvider.notifier).initiateMpesaStk("+254 7XX XXX XXX", 500);
                       },
                       icon: const Icon(Icons.payment, color: Colors.white),
                       label: const Text('Pay Emergency Fee', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
